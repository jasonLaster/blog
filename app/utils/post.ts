...
export function standardizeMetadata(
  ...
  const date =
    (importedMetadata.publishedAt as string) ||
    (importedMetadata.date as string);
  if (!date) {
  ...
  const summary =
    (importedMetadata.description as string) ||
    (importedMetadata.summary as string) ||
    "";
...
export function extractMetadata(filePath: string): Metadata {
  ...
  const postMetadataMatch = content.match(
    /export const metadata = postMetadata\(({[\s\S]*?)\);/
  );
  ...
  if (match) {
    try {
      ...
      console.error(
        `Error processing metadata from ${filePath}: ${
