...
This is totally silly and is probably an area where `@nextjs/mdx` could export a `getPostsMetadata` function that takes care of this for you.
...
After messing around for a couple of minutes, I decided to go with [Bright](https://bright.codehike.org/) which handled these cases fairly well and played nicely with `nextjs/mdx`
